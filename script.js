...
(function() {
        ...
        if (fbProds && fbProds.length > 0) {
          ...
          window.__admRefresh = (products) => {
            ...
            admFiltered = admProducts.filter(p => {
              ...
              return (p.name+' '+(p.brand||'')).toLowerCase().includes(q) && (!cat||p.category===cat);
            });
            renderAdmTable();
            if ($i('admProdBadge')) $i('admProdBadge').textContent = admProducts.length;
          };
  ...
  window.admFilterProducts = function() {
    ...
    const cat = $i('admCatFilter')?.value || '';
    admFiltered = admProducts.filter(p => {
      const nm = (p.name + ' ' + (p.brand||'')).toLowerCase();
      return nm.includes(q) && (!cat || p.category === cat);
    });
    admPage = 1;
    renderAdmTable();
  };
    ...
    tbody.innerHTML = items.map(p => `
      <tr>
        ...
        <td>
          ...
          ${p.sku ? `<div style="font-size:11px;color:#444">SKU: ${p.sku}</div>` : ''}
        </td>
